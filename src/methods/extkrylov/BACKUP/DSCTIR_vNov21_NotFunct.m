...
function [V,KLrot,KLidx,KR,LR] = DSCTIR(A,V,KLrot,KLidx,KR,LR,mu)
	...
	for kk = 1:size(mu,2)
		...
		elseif (KLidx(1) == 1) % Lxx
            if (KLidx(2) == 0) % LKx
                ...
                if (KLidx(3) == 0) % LLK
                    ...
                    [AscBranch, KR, LR, TBranch] = ShiftBranchThroughPencil(AscBranch, KR, LR, 1, n-1);
                    [Limb, DescBranch(:,1),DescBranch(:,2)] = RotST(DescBranch(:,1),DescBranch(:,2), KLrot(:,1));
                    LimbSide = 1;
                    DescBranch(:,2) = RotFUS(DescBranch(:,2), KLrot(:,2));
                    % ready to augment
        ...
        for i=1:n-4
            ...
            V = ApplyRotToV(RotH(AscBranch(:,2)), V, i+2);
            V = ApplyRotToV(RotH(AscBranch(:,1)), V, i+1);
        ...
        [AscBranch(:,1), DescBranch(:,1),DescBranch(:,2)] = RotST(DescBranch(:,1),DescBranch(:,2), AscBranch(:,1));
        % Move limb through Ascending branch to the middle
        [AscBranch(:,1), AscBranch(:,2),Limb] = RotST(Limb,AscBranch(:,1), AscBranch(:,2));
        % Fix CT
        KLrot(:,i) = DescBranch(:,1);
        KLidx(i) = LimbSide;
        % Remove Ascending branch from the left
        V = ApplyRotToV(RotH(AscBranch(:,2)), V, i+1);
        V = ApplyRotToV(RotH(AscBranch(:,1)), V, i+2);
...
function [DBranch, Rstart, Rend, TBranch] = ShiftBranchThroughPencil(DBranch, Rstart, Rend, i, k)
