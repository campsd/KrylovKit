...
function [V,KLrot,KLidx,KR,LR] = CTIR(V,KLrot,KLidx,KR,LR,mu)
	...
	% ---------------------------------------------------------------------
		...
		if (KLidx(1) == 0) % First rotation at K
			if (KLidx(2) == 0) % Second rotation also at K
				...
				[ChaRot, LR] = ShiftRotRightToLeft(ChaRot, LR, 1, n-1); % HERE IT DOESN'T WORK??
                ChaRot = RotH(ChaRot) ;
            end
	...
	% ---------------------------------------------------------------------	
		for i=1:n-3
			if ( KLidx(i) == 0 ) % i -th rotation at K side
				if ( KLidx(i+1) == 0 ) % i+1 -th rotation also at K side
					[ChaRot, KLrot(:,i),KLrot(:,i+1)] = RotST(KLrot(:,i),KLrot(:,i+1), ChaRot) ;
					V = ApplyRotToV(ChaRot, V, i+1);
					if ( KLidx(i+2) == 0 ) % i+2 -th rotation also at K Side
					%	1.1.1		-->	1.1.1, 1.1.2
					%	K	L
					%	|
					%	 |
					%	  |
                        ...
                        ChaRot = RotH(ChaRot) ;
					else % i+2-th rotation swapped to L side
					%	1.1.2		-->	1.2.1, 1.2.2					
					%	K	L
					%	|
					%	 |
				...
				else % i+1 -th rotation swapped to L side
					if ( KLidx(i+2) == 1 ) % i+2 -th rotation still at L side
					%	1.2.1		-->	2.1.1, 2.1.2
					%	K	L
					%	|
					%		|
					%		 |
						warning('I shouldnt be here');
					else % i+2 -th rotation swapped back to K side
					%	1.2.2		-->	2.2.1, 2.2.2
					%	K	L
					%	|
					%		|
			...
			elseif ( KLidx(i) == 1 ) % ith rotation at L side
				if ( KLidx(i+1) == 1 ) % i+1 -th rotation also at L side
					[ChaRot, KLrot(:,i),KLrot(:,i+1)] = RotST(KLrot(:,i),KLrot(:,i+1), ChaRot) ;
					V = ApplyRotToV(ChaRot, V, i+1);
					if ( KLidx(i+2) == 1 ) % i+2 -th rotation also at L side
					%	2.1.1		-->	2.1.1, 2.1.1
					%	K	L
					%		|
					%		 |
					%		  |		
						...
						[ChaRot, LR] = ShiftRotRightToLeft(ChaRot, LR, i+1, n-1);
                        ChaRot = RotH(ChaRot) ;
					else % i+2 -th rotation swapped to K side
					%	2.1.2		-->	2.2.1, 2.2.2
					%	K	L
					%		|
					%		 |
				...
				else % i+1 -th rotation swapped to K side
					if ( KLidx(i+2) == 0 ) % i+2 -th rotation still at K side
					%	2.2.1		-->	1.1.1, 1.1.2
					%	K	L
					%		|
					%	|
					%	 |			
						warning('I shouldnt be here');		
					else % i+2 -th rotation swapped back to L side
					%	2.2.2		-->	1.2.1, 1.2.2
					%	K	L
					%		|
					%	|
