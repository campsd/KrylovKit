...
if size(LR,1) == size(LR,2)+1,
    LR(end-1:end,end) = CreateRotMat(KLrot(:,end)) * LR(end-1:end,end); % last rotation at L
    LR(end,:) = [];
...
for k=n-1:-1:1
    if(KLidx(k) == 1) % L rotation, apply left
         PCX2(k:k+1,:) = CreateRotMat(KLrot(:,k)) * PCX2(k:k+1,:);
    elseif (KLidx(k) == 0) && (k>i)% K rotation, apply H right
         PCX2(:,k-i:k-i+1) = PCX2(:,k-i:k-i+1) * CreateRotMat(RotH(KLrot(:,k)));
    end
end

PC21 = zeros(j,size(PC,2));
if i > 0, 
    G = CreateRotMat(KLrot(:,i)) * [LKi(end,end);0];
    PC21(1,end) = G(2);
    if(KLidx(i-1) ~= 1)
        k = i-1;
        while (KLidx(k) == 0)
            PC21(1,k:k+1) = PC21(1,k:k+1) * CreateRotMat(RotH(KLrot(:,k)));
            k = k - 1; 
        end
    end
end
