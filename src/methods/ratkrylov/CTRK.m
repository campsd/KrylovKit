...
%   LR      upper triangular from L
    ...
    for i=1:k
        ...
        elseif Xi{i} == 0 %BA-1v
            ...
            Krot(:,curr_i) = [conj(ck); -sk];
        end
    end
    
    function ApplyKrot(curr_i)
        % Applies the rotations in Lrot to li
        for ii=1:curr_i-1
            ki(ii:ii+1) = CreateRotMat(RotH(Krot(:,ii))) * ki(ii:ii+1);
        end
    end
    
    function ApplyLrot(curr_i)
        % Applies the rotations in Lrot to li
        for ii=1:curr_i-1
            li(ii:ii+1) = CreateRotMat(RotH(Lrot(:,ii))) * li(ii:ii+1);
        end
    end    
end